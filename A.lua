			...
			if _G.BringMonster then
				...
				for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
					...
					if _G.AutoFarmGunMastery and StartMasteryGunMagnet then
						...
						elseif v.Name == Mon thenjust nhatz
							if (v.HumanoidRootPart.Position - PosMonMasteryGun.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
								v.Humanoid:ChangeState(14)
								v.HumanoidRootPart.CanCollide = false
								v.Head.CanCollide = false
...
spawn(function()
	while wait() do
		if  _G.Khi_Dot_Len_Dien and nhatz then
			pcall(function()
...
spawn(function()
	while wait(.5) do
		pcall(function()
			if _G.Gan_Duoc_Cho_Khi and maythgchongu then
...
spawn(function()
	while wait(.2) do
		pcall(function()
			if Gan_Haki_Lon and Syntaxz  then
...
local Toggle = Tabs.item:AddToggle("auto Kill", {
	...
	Description = "chỉ đánh  th chứ m mong gì nx ",
	Default = false
})
Toggle:OnChanged(function(Value)
