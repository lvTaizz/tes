...
function CheckQuest() 
    ...
    if World1 then
        ...
        elseif MyLevel == 375 or MyLevel <= 399 then
            ...
            if _G.MethodFarm and (CFrameQuest.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 10000 then
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- Get essential game services and properties
            ...
            -- Exit if key components are missing
            if not humanoidRootPart or not enemiesFolder then return end

            -- Check global flags before proceeding
            if _G.BringMonster and StartBring then
                for _, enemy in pairs(enemiesFolder:GetChildren()) do
                    -- Validate enemy model and required parts
                    if enemy:IsA("Model") and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") then
                        local enemyHRP = enemy.HumanoidRootPart
                        local distance = (enemyHRP.Position - humanoidRootPart.Position).Magnitude

                        -- Determine if the enemy matches the target
                        local isTargetEnemy = 
                            (enemy.Name == "Factory Staff" and distance <= 250) or
                            ((enemy.Name == MonFarm or enemy.Name == Mon) and distance <= 330)
                        ...
                        if isTargetEnemy then
                            ...
                            enemyHRP.CFrame = PosMon

                            -- Remove the animator to prevent animations
                            local animator = enemy.Humanoid:FindFirstChild("Animator")
                            if animator then
                                animator:Destroy()
                            end

                            -- Set the simulation radius to affect nearby objects
...
-- Function to determine if an object is in the network range
function InMyNetWork(object)
    if typeof(object) ~= "Instance" or not object:IsA("BasePart") then return false end

    local player = game:GetService("Players").LocalPlayer
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")

    -- Use network ownership if available, otherwise fallback to distance check
    if isnetworkowner then
        return isnetworkowner(object)
    elseif hrp then
        return (object.Position - hrp.Position).Magnitude <= 330
    end
    return false
end
...
AutoFarm = Tabs.Main:AddToggle("MyToggle", {Title = "Auto Level", Default = false })
...
-- Combined and Optimized Auto Bone Farming Script

Auto_Bone = Tabs.Main:AddToggle("MyToggle", {Title = "Auto Bone", Default = false })
Auto_Bone:OnChanged(function(Value)
    _G.Auto_Bone = Value
    StopTween(_G.Auto_Bone)
end)
spawn(function()
    while wait(0.5) do
       local boneframe = CFrame.new(-9502.462890625, 172.13986206054688, 6063.9736328125)
        
        if _G.Auto_Bone and World3 then
            pcall(function()
                local playerPos = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                local distance = (playerPos - boneframe.Position).Magnitude
                ...
                if foundMob then
                    for _, v in pairs(enemies:GetChildren()) do
                        if table.find(targetMobs, v.Name) and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            repeat
                                task.wait(0.1)
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                    ...
                    for _, v in pairs(enemies:GetChildren()) do
                        if table.find(targetMobs, v.Name) and v:FindFirstChild("HumanoidRootPart") then
            ...
            -- If Auto_Bone is false, ensure to stop any ongoing tweening or actions
            StopTween(true)  -- Call StopTween to stop any ongoing actions
        end
    end
end)